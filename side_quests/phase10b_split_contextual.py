#!/usr/bin/env python3
"""
Phase 10b: Split Ratio Window - Condition B (Contextual)
Same fixed seed, same architecture. Varies only split ratio.
"""
import os
import torch
...
# Main
print("Phase 10b: Split Ratio Window - Condition B (Contextual)")
print(f"Fixed seed: {FIXED_SEED}")
print()

images, labels = load_contextual_data()
...
for split in SPLITS:
    ...
    results[split_name] = {'train_size': len(train_idx), 'val_size': len(val_idx), 'val_accuracy': acc}
    print(f"  {split_name}: train={len(train_idx)}, val={len(val_idx)}, accuracy={acc:.4f}")

print(f"\nSplit ratio window (Condition B):")
for name, r in results.items():
    print(f"  {name}: {r['val_accuracy']:.4f}")

# Load Condition A results for comparison
...
for split_name in results:
    a_acc = results_a[split_name]['val_accuracy']
    b_acc = results[split_name]['val_accuracy']
    print(f"  {split_name:<12} {a_acc:<10.4f} {b_acc:<10.4f} {b_acc-a_acc:+.4f}")

with open('models/split_window_contextual.json', 'w') as f:
    json.dump(results, f, indent=2)
print("\nSaved to models/split_window_contextual.json")
